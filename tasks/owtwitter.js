...
module.exports = {
  run: async (client) => {
    const keywords = ["patch", "bug", "tweaks", "balance", "changes"];
    const cavalryUrl = "https://nitter.net/OWCavalry";
    const cavalryPage = await fetch(cavalryUrl, { redirect: "follow", follow: 100 })
      .then(function (res) {
        if (res.status == 200) {
          return res.text();
        }
    ...
    tweets.forEach((t) => {
      var bodyText;
      const body = t.childNodes.filter(el => el.rawAttrs == "class=\"tweet-content media-body\" dir=\"auto\"");
      ...
      if (bodyText && bodyText.length && !keywords.some(k => bodyText.includes(k))) return;

      const linkEl = t.parentNode.childNodes.filter(el => el.rawAttrs && el.rawAttrs.includes("tweet-link"))[0].rawAttrs;
      const link = "https://twitter.com" + linkEl.substring(25, linkEl.length - 3);
